...
class AdresseFetcher(BaseFetcher):
    ...
    def geocode_single(self, address: str) -> GeocodingResult:
        ...
        Retourne un GeocodingResult avec score.
        """
        if not address or address.strip() == "":
            return GeocodingResult(original_address=address or "", score=0.0)

        try:
            data = self._make_request("/search/", params={"q": address, "limit": 1})

            if not data.get("features"):
                return GeocodingResult(original_address=address, score=0.0)
    ...
    def fetch_batch(self, addresses: list[str]) -> list[GeocodingResult]:
        ...
        Applique rate limiting entre chaque requête.
        """
        results = []
    ...
    def fetch_all(
        ...
        """
        Géocode toutes les adresses (avec barre de progression).
        """
